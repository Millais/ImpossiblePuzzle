...
1. <1747 elements in the list binding Q after executing s1(Q,100)>
    S1 first computes a list of all possibilities in the format [X,Y,Sum,Product], 
    with the constraints 1 < X <Y and X+Y=100. The list is then sorted by product,
    and filtered so that only duplicate products remain (unique 
    products are removed). Finally the list is sorted in order of sum.

2. <145 elements in the list binding Q after executing s2(Q,100)>
    S2 first performs the same way as S1, but notes a list of sums which correspond
    to unique products. These sums are then ordered, duplicates removed, and then
    a predicate removes quads with these corresponding sums from the list
    generated in S1. Finally, the list is ordered by product.

3. <86 elements in the list binding Q after executing s3(Q,100)>
    S3 uses the list of quads [X,Y,S,P] generated from S2 and removes quads which 
    have the same product value. The quads are then sorted by sum value.

4.  S4 uses the list of quads generated from S3 and then removes any quads which 
    have the same sum value appearing more than once. Finally, the result can be
    reached.

5. <1 element in the list binding Q after executing s4(Q,500)>
    s4(Q,500) uses 5983939 inferences. */


/* For S1 only, a list of unique sums is not required and so this is delimited
by a '_' in the remove_unique_products predicate definition.*/

s1(Output, Limit):- 
  generate_quads(2, 3, Limit, Quads),
  merge_sort_product(Quads, MergeResults),
  remove_unique_products(MergeResults, NonUniques, _),
  merge_sort_sum(NonUniques, Output),
  !.

s2(Output, Limit):- 
  generate_quads(2, 3, Limit, Quads),
  merge_sort_product(Quads, MergeResults),                         
  remove_unique_products(MergeResults, NonUniques, DuplicatedUniqueSums),      
  merge_sort_sum(NonUniques, List),                                   
  merge_sort_single(DuplicatedUniqueSums, OrderedDuplicatedUniqueSums), 
  remove_duplicate_singles(OrderedDuplicatedUniqueSums, UniqueSums),          
  remove_corresponding_sums(List, UniqueSums, UnsortedOutput),                             
  merge_sort_product(UnsortedOutput, Output),
  !.

s3(Output, Limit) :-
  s2(Q, Limit),
  remove_duplicate_products(Q, NoDuplicateProducts),
  merge_sort_sum(NoDuplicateProducts, Output),
  !.

s4(Output, Limit) :-

  s3(Q, Limit),
  remove_duplicate_sums(Q, Output),
  !.

% Generates a list of quads (tuples) in the format [X,Y,S,P] where S corresponds
% to the Sum and P corresponds to the product. The quads follow the constraints
% that X < Y and X+Y < Limit.

generate_quads(X, Y, Limit, [[X, Y, S, P] | Tail]):-
X + Y =< Limit, !,
S is X + Y,
P is X * Y,
NextY is Y + 1,
generate_quads(X, NextY, Limit, Tail).

generate_quads(X, _, Limit, Tail):-
NextX is X + 1,
NextY is NextX + 1,
NextX + NextY =< Limit, !,
generate_quads(NextX, NextY, Limit, Tail).

generate_quads(_, _, _, []):-!.

% Splits a list into two separate sublists which contain different elements of
% the original list passed in. Split is used by all merge-sort predicates.
split([], [], []).
split([A],[A],[]).
split([A, B | Tail], [A | Tail1], [B | Tail2]) :- split(Tail, Tail1, Tail2).

% Sorts a list of quads into product order (ascending). Various different cases are
% introduced to optimise the speed of the algorithm and to reduce the inference count.
merge_sort_product([], []) :- !.
merge_sort_product([A], [A]) :- !.
merge_sort_product([[X1, Y1, S1, P1],[X2, Y2, S2, P2]], [[X1, Y1, S1, P1],[X2, Y2, S2, P2]]) :- P1 =< P2.
merge_sort_product([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X1, Y1, S1, P1],[X3, Y3, S3, P3],[X2, Y2, S2, P2]]) :- P1 =< P3, P3 =< P2.
merge_sort_product([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X2, Y2, S2, P2],[X1, Y1, S1, P1],[X3, Y3, S3, P3]]) :- P2 =< P1, P1 =< P3.
merge_sort_product([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X2, Y2, S2, P2],[X3, Y3, S3, P3],[X1, Y1, S1, P1]]) :- P2 =< P3, P3 =< P1.
merge_sort_product([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X3, Y3, S3, P3],[X1, Y1, S1, P1],[X2, Y2, S2, P2]]) :- P3 =< P1, P1 =< P2.
merge_sort_product([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X3, Y3, S3, P3],[X2, Y2, S2, P2],[X1, Y1, S1, P1]]) :- P3 =< P2, P2 =< P1.
merge_sort_product([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]]).
merge_sort_product(I, SortedList) :-
  split(I, List1, List2),
  merge_sort_product(List1, Sorted1),
  merge_sort_product(List2, Sorted2),
  merge_products(Sorted1, Sorted2, SortedList).

% Merges the two lists together, resulting in a list ordered by value of product
% in ascending order.
merge_products([], List, List).
merge_products(List, [], List).
merge_products([[X, Y, S, P] | Left], [[X1, Y1, S1, P1]|Right], [[X, Y, S, P] | Tail]) :-
 P =< P1,    
 merge_products(Left, [[X1, Y1, S1, P1] | Right],  Tail).
merge_products([[X, Y, S, P] | Left], [[X1, Y1, S1, P1] | Right], [[X1, Y1, S1, P1] | Tail] ) :-
 P > P1,   
 merge_products([[X, Y, S, P] | Left], Right, Tail).

% Sorts a list of quads into sum order (ascending). Various different cases are
% introduced to optimise the speed of the algorithm and to reduce the inference count.
merge_sort_sum([], []) :- !.
merge_sort_sum([A], [A]) :- !.
merge_sort_sum([[X1, Y1, S1, P1],[X2, Y2, S2, P2]], [[X1, Y1, S1, P1],[X2, Y2, S2, P2]]) :- S1 =< S2.
merge_sort_sum([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X1, Y1, S1, P1],[X3, Y3, S3, P3],[X2, Y2, S2, P2]]) :- S1 =< S3, S3 =< S2.
merge_sort_sum([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X2, Y2, S2, P2],[X1, Y1, S1, P1],[X3, Y3, S3, P3]]) :- S2 =< S1, S1 =< S3.
merge_sort_sum([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X2, Y2, S2, P2],[X3, Y3, S3, P3],[X1, Y1, S1, P1]]) :- S2 =< S3, S3 =< S1.
merge_sort_sum([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X3, Y3, S3, P3],[X1, Y1, S1, P1],[X2, Y2, S2, P2]]) :- S3 =< S1, S1 =< S2.
merge_sort_sum([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X3, Y3, S3, P3],[X2, Y2, S2, P2],[X1, Y1, S1, P1]]) :- S3 =< S2, S2 =< S1.
merge_sort_sum([[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]], [[X1, Y1, S1, P1],[X2, Y2, S2, P2],[X3, Y3, S3, P3]]).
merge_sort_sum(I, SortedList) :-
  split(I, List1, List2),
  merge_sort_sum(List1, Sorted1),
  merge_sort_sum(List2, Sorted2),
  merge_sums(Sorted1, Sorted2, SortedList).

% Merges the two lists together, resulting in a list ordered by value of sums
% in ascending order.
merge_sums([], List, List ).
merge_sums(List, [], List ).
merge_sums([[X, Y, S, P] | Left], [[X1, Y1, S1, P1] | Right], [[X, Y, S, P] | Tail] ) :-
 S =< S1,    
 merge_sums(Left,  [[X1, Y1, S1, P1] | Right],  Tail).
merge_sums([[X, Y, S, P] | Left],  [[X1, Y1, S1, P1] | Right],  [[X1, Y1, S1, P1] | Tail] ) :-
 S > S1,   
 merge_sums([[X, Y, S, P] | Left], Right, Tail).

% Sorts a list of non-tuples into ascending order. (e.g [2,1,5,4] -> [1,2,4,5]).
% Various different cases are introduced to optimise the speed of the algorithm 
% and to reduce the inference count.
merge_sort_single([], []).
merge_sort_single([X], [X]).
merge_sort_single([X,Y],  [X, Y]) :- X =<Y.
merge_sort_single([X, Y, Z],  [X, Z, Y]) :- X =< Z, Z =< Y.
merge_sort_single([X, Y, Z],  [Y, X, Z]) :- Y =< X, X =< Z.
merge_sort_single([X, Y, Z],  [Y, Z, X]) :- Y =< Z, Z =< X.
merge_sort_single([X, Y, Z],  [Z, X, Y]) :- Z =< X, X =< Y.
merge_sort_single([X, Y, Z],  [Z, Y, X]) :- Z =< Y, Y =< X.
merge_sort_single([X, Y, Z],  [X, Y, Z]).
merge_sort_single(U, S) :- 
  split(U, L, R), 
  merge_sort_single(L, SL), 
  merge_sort_single(R, SR), 
  merge_single(SL, SR, S).
 
% Merges the two lists together, resulting in a list ordered by value.
merge_single([], RS, RS ).
merge_single(LS, [], LS ).
merge_single([L | LS], [R | RS], [L | T] ) :- L =< R, merge_single(LS, [R | RS], T).
merge_single([L | LS], [R | RS], [R | T] ) :- L > R,  merge_single([L | LS], RS, T).

% Removes quads from the list passed in which have unique products. 'Results' is
% the list of quads containing only duplicates - no unique products - whereas
% 'Output' is the list of sums correponding to unique products.

remove_unique_products([], [], []).
remove_unique_products([X, Y, Z|Tail], [X | Results],  Output)   :- 
  [_, _, _, P] = X,  
  [_, _, _, P] = Y,  
  [_, _, _, P] = Z,  
  remove_unique_products([Y, Z | Tail], Results,  Output).
remove_unique_products([X, Y | Tail], [X, Y | Results],  Output)   :-  
  [_, _, _, P] = X,  
  [_, _, _, P] = Y,  
  remove_unique_products(Tail, Results,  Output).
remove_unique_products([H | Tail], Results,  [S | Output]) :- 
  [_, _, S, _] = H, 
  remove_unique_products(Tail, Results, Output).

% Removes duplicates from a list. (e.g [1,2,2,3,3,4,4,5] -> [1,2,3,4,5])
remove_duplicate_singles([], []).
remove_duplicate_singles([X], [X]).
remove_duplicate_singles([X, X | Tail], Results) :- 
  remove_duplicate_singles([X | Tail], Results).
remove_duplicate_singles([X, Y | Tail2], [X | Results]) :-
  X \= Y,
  remove_duplicate_singles([Y | Tail2], Results).

% Removes quads in a list which share the same product. Only quads with unique
% product values will remain in the 'Results' list.
remove_duplicate_products([], []) :-!.
remove_duplicate_products([[_, _, _, P], [X1, Y1, S1, P], [X2, Y2, S2, P] | Tail], Results)   :- 
  remove_duplicate_products([[X1, Y1, S1, P], [X2, Y2, S2, P] | Tail], Results).
remove_duplicate_products([[_, _, _, P], [_, _, _, P] | Tail], Results)   :-  
  remove_duplicate_products(Tail, Results).
remove_duplicate_products([H | Tail], [H | Results]) :- 
  remove_duplicate_products(Tail, Results).

% Removes quads in a list which share the same sum. Only quads with unique
% sum values will remain in the 'Results' list.
remove_duplicate_sums([], []) :-!.
remove_duplicate_sums([[_, _, S, _], [X1, Y1, S, P1], [X2, Y2, S, P2] | Tail], Results)   :- 
  remove_duplicate_sums([[X1, Y1, S, P1], [X2, Y2, S, P2] | Tail], Results).
remove_duplicate_sums([[_, _, S, _], [_, _, S, _] | Tail], Results)   :-  
  remove_duplicate_sums(Tail, Results).
remove_duplicate_sums([H | Tail], [H | Results]) :- 
  remove_duplicate_sums(Tail, Results).

% The predicate takes a list of ordered quads (by sum) and a list of ordered unique
% sums. The list of quads is traversed in terms of the unique sums - when a sum 
% matches, the quad is not added to the list returned. If the sum does not match
% the current position in the list of unique sums, the position of the quads list
% is continually adjusted until a match is found/the list is empty, with 
% non-matching quads added to the final output. The final 'Output' contains quads 
% with no sums which will have been present in the list of sums corresponding to unique products.

remove_corresponding_sums([], _,  []) :- !.
remove_corresponding_sums([[_, _, S, _]], [S | _],  []) :- !.

remove_corresponding_sums([[_, _, S, _]  |  Tail],  [S  |  Sums],  Output) :-
  remove_corresponding_sums(Tail,  [S |  Sums],  Output),  !.

remove_corresponding_sums([[X, Y, S1, P]  |  Tail],  [S  |  Sums],  Output) :-
  S1 > S, 
  remove_corresponding_sums([[X, Y, S1, P]  | Tail],  Sums,  Output), !.

remove_corresponding_sums([X  |  Tail],  [S  |  Sums],  [X  |  Output]) :-
  remove_corresponding_sums(Tail,  [S  | Sums],  Output),  !.

/*
?- consult(pm515).
% pm515 compiled 0.06 sec, 74 clauses
true.

?- time(s1(Q,100)).
% 136,822 inferences, 0.172 CPU in 0.208 seconds (82% CPU, 796055 Lips)
...
?- time(s2(Q,100)).
% 157,796 inferences, 0.188 CPU in 0.249 seconds (75% CPU, 841579 Lips)
...
?- time(s3(Q,100)).
% 159,296 inferences, 0.188 CPU in 0.236 seconds (79% CPU, 849579 Lips)
...
?- time(s4(Q,100)).
% 159,375 inferences, 0.219 CPU in 0.233 seconds (94% CPU, 728571 Lips)
Q = [[4, 13, 17, 52]].

?- time(s4(Q,500)).
% 5,983,940 inferences, 16.797 CPU in 19.690 seconds (85% CPU, 356253 Lips)
Q = [[4, 13, 17, 52]].
*/